...
void List::add(int inData) { // YOU are never changing the data
    Node *ptrNewNode = new Node; // create a new node global for 2 or more nodes
    ptrNewNode->data = inData;   // point New node to data being entered (data)
//  if( ptrHead == NULL) {
    if(isEmpty()) {
        ptrHead = new Node;
        ptrHead->data = inData;
    ...
    } else { // condition more than 2 nodes
        Node *ptrCurrent = ptrHead; // temp var to iterate through while loop // ptrCurrent = ptrHead to compare to the rest of the nodes
        Node *ptrPrevious = NULL; // this tells us that we're starting at the head of the linked list as the previous of the head is always NULL
        while(ptrCurrent->ptrNext != NULL) { // 13<45 23 // when this iterates through the while loop this loop will NOT check the tail
            if(ptrNewNode->data < ptrCurrent->data){ // if the new number you're adding is the less than the current number // inside conditionals you can use the data to compare
                if(ptrCurrent == ptrHead){
                    ptrNewNode->ptrNext = ptrHead; // assigning the new node to the head
                    ptrHead = ptrNewNode; // then reassigning the new node as the head
                    break;
                }
            ...
            ptrPrevious = ptrCurrent; // counter // line 53-54 is the iterator that moves through the linked-list
            ptrCurrent = ptrCurrent->ptrNext;
            if(ptrCurrent->ptrNext == NULL) { // this is checking for the tail
                if(ptrNewNode->data < ptrCurrent->data){ // 13 < 23
                    ptrPrevious->ptrNext = ptrNewNode;
                    ptrNewNode->ptrNext = ptrCurrent;
                    break;
                }
